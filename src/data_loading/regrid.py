...
def _grid_1d(start_b: float, end_b: float, step: float) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def std_transform(with_bounds=True):
    """
...
def regrid_2d(ds_input: Union[xr.Dataset, xr.DataArray], method: str = "bilinear", periodic: bool = True) -> Union[xr.Dataset, xr.DataArray]:
    """
...
def regrid_2d_to_standard(da: Union[xr.Dataset, xr.DataArray]) -> Union[xr.Dataset, xr.DataArray]:
...
def regrid_1d(ds_input: Union[xr.Dataset, xr.DataArray], method: str = "bilinear", periodic: bool = True) -> Union[xr.Dataset, xr.DataArray]:
    """
    Regrid to a 1x1 grid, using by default bilinear interpolation and periodic boundaries.
...
def regrid_1d_to_std(da: Union[xr.Dataset, xr.DataArray]) -> Union[xr.Dataset, xr.DataArray]:
