...
def test_inversibility() -> None:
    ...
    # Only looks at the first 100 bytes - there are some other bits which are
    # just reordered.
    byte_num = 100
    ...
    with open(str(TEST_DIREC / "test.x"), "r+b") as f:
        byt2 = f.read(byte_num)
        print(byt2)

    assert np.all([byt1[x] == byt2[x] for x in range(len(byt1))])
